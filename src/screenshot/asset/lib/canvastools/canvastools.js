...
(function webpackUniversalModuleDefinition(root, factory) {
  ...
  else if (typeof exports === 'object') exports['CanvasTools'] = factory();
  else root['CanvasTools'] = factory();
})(this, function() {
    ...
    /******/ [
      ...
      /***/ function(module, exports, __webpack_require__) {
        ...
        var getButtons = function getButtons() {
          ...
          for (var key in ButtonsMap) {
            if (useButton(key)) {
              ...
              html.push(
                '<div class="canvas-tools-btn js-btn" data-panel="' +
                  (btn.panel || '') +
                  '" data-value="' +
                  key +
                  '" data-action="" title="' +
                  btn.name +
                  '">\n\t\t\t<a class="btn-toggle"><i class="canvas-tools-icon__' +
                  key +
                  '"></i></a>\n\t\t\t</div>'
              );
            }
        ...
        var getColorPanel = function getColorPanel() {
          ...
          html +=
            '<span class="color-selected"><i class="js-color-selected" style="background:' +
            color +
            '"></i></span>';
          ...
          for (var i = 0; i < 16; i++) {
            var item =
              '<li class="js-color" style="background:' +
              ColorList[i] +
              '" data-value="' +
              ColorList[i] +
              '"></li>';
        ...
        var getAmbiguity = function getAmbiguity() {
          var ambiguite =
            arguments.length > 0 && arguments[0] !== undefined
              ...
              : 0.5;
          return (
            '<label class="ambiguite-range"><span>\u6A21\u7CCA\u5EA6</span><input type="range" min="0" step="0.01" max="1" value="' +
            ambiguite +
            '" class="js-mosaic-ambiguity"></label>'
          );
        };
      ...
      /***/ function(module, exports, __webpack_require__) {
        ...
        var buildStrokePanel = function buildStrokePanel() {
          ...
          el.innerHTML =
            _template2.default.getStrokePanel(stroke) +
            _template2.default.getColorPanel(color);
          return el;
        };
        ...
        var buildFontPanel = function buildFontPanel() {
          ...
          el.style.cssText += 'white-space: nowrap!important;';
          el.innerHTML =
            _template2.default.getFontPanel(fontSize) +
            _template2.default.getColorPanel(color);
          return el;
        };
        ...
        var buildAmbiguityPanel = function buildAmbiguityPanel() {
          ...
          el.innerHTML =
            _template2.default.getStrokePanel(stroke) +
            _template2.default.getAmbiguity(ambiguity);
          return el;
        };
        ...
        function __bindEvents() {
          ...
          var $btns = _utils2.default.$('.js-btn', $el),
            ...
            $mosaicAmbiguity = _utils2.default.$(
              '.js-mosaic-ambiguity',
              $el
          ...
          _handles.btnEmit = function(event) {
             ...
             * 非功能性操作不做任何反馈
             */
            if (!['ok', 'cancel', 'save'].includes(value)) {
              startEditCanvas();
            }
          ...
          _handles.onMouseDown = function(event) {
            if (
              !!~STROKE_TYPES.indexOf(state.drawType) === false ||
              state.drawType === 'font' ||
              state.drawType === 'drag'
            ) {
              return;
            ...
            state.lastImageData = context.getImageData(
              0,
              0,
              rect.width,
              rect.height
          ...
          _handles.onMouseUp = function(event) {
            ...
            if (
              !!~STROKE_TYPES.indexOf(state.drawType) &&
              state.drawType !== 'font'
            ) {
        ...
        function __drawEllipseByHistory(historyObj) {
          ...
          historyObj.points = [
            {
              startx: obj.startx - this.hmovex,
              starty: obj.starty - this.hmovey,
              posx: obj.posx - this.hmovex,
              posy: obj.posy - this.hmovey
            }
          ];
        }
        ...
        function __drawBrushByHistory(historyObj) {
          ...
          newPoints.push({
            x: start.x - this.hmovex,
            y: start.y - this.hmovey
          });
          for (var i = 1; i < historyObj.points.length; i++) {
            ...
            newPoints.push({
              x: element.x - this.hmovex,
              y: element.y - this.hmovey
            });
          }
          context.stroke();
        ...
        function __drawFont(event) {
          ...
          context.restore();
          context.closePath();
          this.historyDraws.push({
            type: 'font',
            points: [{ x: x, y: y }],
            fillStyle: this.state.strokeColor,
            font: this.state.fontSize + 'px ' + TEXT_FONT_FAMILY,
            content: content
          });
          this.state.isEntry = false;
          removeTextHelper();
        ...
        function __drawMoasicByHistory(historyObj) {
          ...
          historyObj.points.forEach((element) => {
            ...
            context.fillRect(
              element.x - this.hmovex,
              element.y - this.hmovey,
              element.size,
              element.size
            );
            context.restore();
        ...
        function __toggleCanvasCursor() {
          ...
          if (cursor) {
            canvas.className =
              canvas.className.replace(/canvas-cursor__(\w+)/, '').trim() +
              (' canvas-cursor__' + cursor);
          }
        }
        ...
        function __pushHistory() {
          this.history.push(
            this.context.getImageData(0, 0, this.rect.width, this.rect.height)
          );
        }
        /*{
type:"",
points:[]
        ...
        function __pushHistoryDraws() {
          this.historyDraws.push({
            type: this.tempDrawObj.type,
            points: this.tempDrawObj.points,
            strokeStyle: this.tempDrawObj.strokeStyle,
            lineWidth: this.tempDrawObj.lineWidth
          });
          ...
          this.historyDraws.forEach((element) => {
            switch (element.type) {
              case 'rect':
                ...
                this.state.lastImageData = this.context.getImageData(
                  0,
                  0,
                  this.rect.width,
                  this.rect.height
          ...
          _createClass(CanvasTools, [
            {
              ...
              value: function render() {
                ...
                this.$el = el;
                this.$el.appendChild(
                  buildStrokePanel(S.strokeWidth, S.strokeColor)
                );
                this.$el.appendChild(buildFontPanel(S.fontSize, S.strokeColor));
                this.$el.appendChild(
                  buildAmbiguityPanel(S.strokeWidth, S.ambiguity)
                );
